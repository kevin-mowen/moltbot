    ...
    <div class="card">
      ...
      <div class="status-list" style="margin-top: 16px;">
        ...
        <div>
          ...
          <span>${slack?.lastStartAt ? formatRelativeTimestamp(slack.lastStartAt) : t("common.notAvailable")}</span>
        </div>
        <div>
          <span class="label">${t("channels.status.lastProbe")}</span>
          <span>${slack?.lastProbeAt ? formatRelativeTimestamp(slack.lastProbeAt) : t("common.notAvailable")}</span>
        </div>
      </div>
